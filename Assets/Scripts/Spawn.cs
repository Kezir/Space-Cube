...
public class Spawn : MonoBehaviour
{
    ...
    public void SpawnWaveBoss()
    {
        for(int i = 0; i < 3; i++)
        {
            GameObject enemy = Instantiate(enemiesPrefabs[i], new Vector3(0f, 6f, 0f), enemiesPrefabs[i].transform.rotation);
            Enemy _enemy = enemy.GetComponent<Enemy>();
            _enemy.type = "Soldier";
            _enemy.movement = "Free Roam";
            _enemy.targetPosition = new Vector3(0f, 6f, 0f);
            _enemy.speed = 2;
        }
    }
